...
$root_src = "C:\temp\powershell\src"
$root_dst = "C:\temp\powershell\dst"
$files_src = gci -file $root_src -recurse | where {$_.attributes -match 'ReparsePoint'} | select -ExpandProperty fullname
$files_dst = gci -file $root_dst -recurse | select -ExpandProperty fullname
# Measure results, and display count
$files_src | measure | % {$_.count }
$files_dst | measure | % {$_.count }
...
Compare-Object -ReferenceObject $compare_src -DifferenceObject $compare_dst | Where { $_.SideIndicator -eq '<=' }
# Compare-Object. Measure left only (count)
Compare-Object -ReferenceObject $compare_src -DifferenceObject $compare_dst | Where { $_.SideIndicator -eq '<=' } | measure | % {$_.count }