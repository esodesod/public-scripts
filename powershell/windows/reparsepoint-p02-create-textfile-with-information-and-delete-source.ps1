...
foreach ($file in $files_dst) {
    ...
    if ($file.Attributes -match "ReparsePoint") {
        ...
        $header | Out-File $outfile -Append -WhatIf
        Write-Output "TECHNICAL INFORMATION - About the ReparsePoint file" | Out-File $outfile -Append -WhatIf
        # Outfile: Append information to outfile
        $file | Format-List * | Out-File $outfile -Append -WhatIf
        # Check if outfile exists, and if so, remove the ReparsePoint
        $outfile_check = (Test-Path $outfile)
